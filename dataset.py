...
class PoseSegDataset(Dataset):
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
        ...
        if abnormal_train_path is not None:
            ...
            ab_labels = get_ab_labels(self.segs_data_np_ab, self.segs_meta_ab, path_to_vid_dir, abnormal_train_path, dataset_args.get('dataset', 'ShanghaiTech'))
    ...
    for person_dict_fn in tqdm(json_list):
        ...
        #! AIHub custom 수정 !#
            ...
            type, class_id, scene_id, clip_id, id2, id3 = \
                re.findall('C(\d+)_A(\d+)_SY(\d+)_P(\d+)_S(\d+)_(.*)_alphapose_.*', person_dict_fn)[0]
